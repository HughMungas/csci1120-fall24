...
template <class elemType>
int reqBinarySearch(const elemType list[], const elemType &item, int start, int end)
{
    int mid = (start + end) / 2;
    ...
    if (list[mid] != item) {
        if (list[mid] < item) return reqBinarySearch(list, item, mid + 1, end);

        if (list[mid] > item) return reqBinarySearch(list, item, start, end - 1);
    }
